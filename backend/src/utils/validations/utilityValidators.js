...
// Schema for cashpower requests
export const cashpowerSchema = Yup.object({
  meterNumber: Yup.string()
    .required('Meter number is required')
    .trim()
    .matches(/^[0-9]{10,12}$/, 'Invalid meter number format'),

  amount: Yup.number()
    .required('Amount is required')
    .positive('Amount must be positive')
    .min(50, 'Minimum amount is D50')
    .max(5000, 'Maximum amount is D5000'),
...
export const statusQuerySchema = Yup.object({
  ...
  provider: Yup.string()
    .oneOf(['africell', 'qcell', 'comium', 'gamcel', 'nawec', 'gamswitch'], 'Invalid provider'),

  type: Yup.string()
    .oneOf(['airtime', 'cashpower'], 'Invalid transaction type'),
